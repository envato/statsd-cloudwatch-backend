...
_.extend(Backend.prototype, {
  ...
  collect_timers: function(date, timers, dimensions) {
    var metrics = []
    _.each(this.filter_metrics(timers), function(item){
      var data = item.value.length
        ? item.value : [0]
      ...
      metrics.push({ MetricName: item.key, StatisticValues: values, Unit: 'Milliseconds',
  ...
  collect_counters: function(date, counters, dimensions) {
    var metrics = []
    _.each(this.filter_metrics(counters), function(item){
      metrics.push({ MetricName: item.key, Value: item.value, Unit: 'Count',
        Timestamp: date, Dimensions: dimensions
      })
    })
  ...
  collect_gauges: function(date, gauges, dimensions) {
    var metrics = []
    _.each(this.filter_metrics(gauges), function(item){
      metrics.push({ MetricName: item.key, Value: item.value, Unit: 'None',
        Timestamp: date, Dimensions: dimensions
      })
    })
    return metrics
